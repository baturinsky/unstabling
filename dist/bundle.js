(() => {
  ...
  for (let c2 in Constants) {
    Flatten[c2] = Constants[c2];
  }
  ...
  function booleanOpBinary(polygon1, polygon2, op, restore2) {
    let res_poly = polygon1.clone();
    let wrk_poly = polygon2.clone();
    let intersections = getIntersections(res_poly, wrk_poly);
    sortIntersections(intersections);
...
M${this.first.start.x},${this.first.start.y}`;
  ...
  var Matrix = class {
    constructor(a = 1, b2 = 0, c2 = 0, d = 1, tx = 0, ty = 0) {
      this.a = a;
      this.b = b2;
      this.c = c2;
      this.d = d;
...
<path stroke="${stroke || "black"}" stroke-width="${strokeWidth || 1}" fill="${fill || "lightcyan"}" fill-rule="${fillRule || "evenodd"}" fill-opacity="${fillOpacity || 1}" ${id_str} ${class_str} d="`;
  ...
  var Distance = class {
    ...
    static minmax_tree_process_level(shape, level2, min_stop, tree) {
      let mindist, maxdist;
      for (let node of level2) {
      ...
      if (level2.length === 0)
        return min_stop;
      let new_level_left = level2.map((node) => node.left.isNil() ? void 0 : node.left).filter((node) => node !== void 0);
      let new_level_right = level2.map((node) => node.right.isNil() ? void 0 : node.right).filter((node) => node !== void 0);
    ...
    static minmax_tree(shape, set, min_stop) {
      ...
      let level2 = [set.index.root];
      let squared_min_stop = min_stop < Number.POSITIVE_INFINITY ? min_stop * min_stop : Number.POSITIVE_INFINITY;
      squared_min_stop = Distance.minmax_tree_process_level(shape, level2, squared_min_stop, tree);
      return tree;
    }
  ...
  // src/render.ts
  var { Point: Point2, Vector: Vector2, Circle: Circle2, Line: Line2, Ray: Ray2, Segment: Segment2, Arc: Arc2, Box: Box2, Polygon: Polygon2, Matrix: Matrix2, PlanarSet: PlanarSet2 } = main_esm_default;
  ...
  var boardCenter = new Point2(canvas.width / 2, canvas.height / 2);
  var mousePos = new Point2(0, 0);
  var dragging;
  var idt = c.createImageData(2, 2);
  idt.data.set([0, 0, 0, 255, 255, 255, 255, 255, 255, 255, 255, 255, 0, 0, 0, 255]);
  var idtc = document.createElement("canvas");
  idtc.width = idtc.height = 2;
  var idtcc = idtc.getContext("2d");
  idtcc.putImageData(idt, 0, 0);
  var xpat = c.createPattern(idtc, "repeat");
  function renderBall(x, y, blocked = false, hover = false) {
  ...
  function render() {
    ...
    c.fillStyle = "#000";
    c.beginPath();
    c.arc(cr.x, cr.y, r, 0, Math.PI * 0.5);
    c.lineTo(cr.x, cr.y);
  ...
  canvas.addEventListener("mousemove", (e) => {
    mousePos = new Point2(e.offsetX - boardCenter.x, e.offsetY - boardCenter.y);
    let node = nodeAt(mousePos);
    if (!dragging) {
  ...
  // src/index.ts
  var { Point: Point3, Vector: Vector3, Circle: Circle3, Line: Line3, Ray: Ray3, Segment: Segment3, Arc: Arc3, Box: Box3, Polygon: Polygon3, Matrix: Matrix3, PlanarSet: PlanarSet3 } = main_esm_default;
  function fromAngle(a, l = 1) {
    return new Point3(Math.cos(a) * l, Math.sin(a) * l);
  }
  var b;
  var level;
  var aiMoving;
  function addNode(nodes, lane, at) {
  ...
  function createBoard(segs, balls, stableRadius = 40) {
    let lanes = segs.map((seg, id) => ({ id, seg, nodes: [] }));
    let nodes = [];
    ...
    lanes.forEach((l) => {
      l.nodes.sort((a, b2) => a.at.distanceTo(l.seg.start)[0] - b2.at.distanceTo(l.seg.start)[0]);
    ...
    b = { nodes, lanes, mass, stableRadius, history: [] };
    calculateCenter();
  }
  ...
  function calculateCenter() {
    ...
    b.center = c2.multiply(1 / b.mass);
  }
  function center2(from, to) {
    return new Vector3(to.x - from.at.x, to.y - from.at.y).multiply(1 / b.mass).add(b.center);
  }
  function aiThink2() {
    let ct = b.center;
    let bestD = 1e6;
  ...
  function aiMove(from, to) {
    ...
    for (let i = 0; i < 1; i += 1 / 20) {
      setTimeout(() => {
        aiMoving.at = new Point3(from.at.x * (1 - i) + to.at.x * i, from.at.y * (1 - i) + to.at.y * i);
        render();
      }, i * 200);
    }
    setTimeout(() => {
  ...
  function state() {
    ...
    return { nodes, lastMoved: b.lastMoved?.id };
  }
  function restore(s) {
    b.nodes.forEach((n, i) => n.mass = s.nodes[i]);
    b.lastMoved = b.nodes[s.lastMoved];
  }
  function starPts(sides, r) {
    return rangef(sides, (i) => fromAngle(Math.PI * (2 / sides * i + 1 / 2), r));
  }
  function starSegs(sides, r, step = 2) {
    let pts = starPts(sides, r);
    return pts.map((p, i) => new Segment3(p, pts[(i + step) % sides]));
  }
  function listPts(ptxy) {
    return ptxy.map(([x, y]) => new Point3(x, y));
  }
  function listSegs(ptxy, sgab) {
    let pts = listPts(ptxy);
    let segs = sgab.map(([a, b2]) => new Segment3(pts[a], pts[b2]));
    return segs;
  }
  function grid(cols, rows, size) {
    let c2 = new Vector3(size * (cols - 1) / 2, size * (rows - 1) / 2);
    let cols1 = rangef(cols, (i) => [[i * size - c2.x, -c2.y], [i * size - c2.x, c2.y]]).flat(1);
    let rows1 = rangef(rows, (i) => [[-c2.x, i * size - c2.y], [c2.x, i * size - c2.y]]).flat(1);
    let pts = [...cols1, ...rows1].map(([x, y]) => new Point3(x, y));
    let sgs = rangef(cols + rows, (i) => new Segment3(pts[i * 2], pts[i * 2 + 1]));
    return sgs;
  }
  grid(2, 3, 10);
  var levels = [
    ["Triangle", 1, () => createBoard(listSegs([[0, -150], [-200, 0], [0, 0], [200, 0], [200, 100]], [[0, 1], [1, 3], [0, 2], [0, 3]]), [0, 1, 2], 75)],
    ["Square", 2, () => createBoard(listSegs([[200, 0], [0, -200], [-200, 0], [0, 200], [0, 0]], [[0, 1], [1, 2], [2, 3], [3, 0], [0, 4], [4, 2]]), [1, 3], 80)],
    ["Bird", 3, () => createBoard(listSegs([[-200, 40], [-100, 0], [0, 40], [100, 0], [200, 40], [-200, -40], [200, -40]], [[0, 1], [1, 2], [2, 3], [3, 4], [2, 5], [2, 6]]), [0, 2, 4], 120)],
